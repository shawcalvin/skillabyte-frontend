...
export default async function QuizReviewPage({
    ...
    params: { courseId: string, attemptId: string }
}) {
    ...
    const attempt = attempts.find(attempt => attempt.id === parseInt(params.attemptId));

    if (!attempts.some(attempt => attempt.score >= 70) || !attempt) {
        return (
            <PermissionDeniedPage courseId={params.courseId} title={quiz.course.title} />
        )
    }
            ...
            <div className="flex flex-col items-center text-primary-blue-900">
                <div className="w-full max-w-7xl p-2">
                    ...
                    <div className="w-full flex justify-center">
                        <Button href={`/learner/courses/${params.courseId}/preview`} className="mt-16">
                            Return to Preview
                        </Button>
                    </div>
                </div>
...
function PermissionDeniedPage({ courseId, title }: { courseId: string; title: string }) {
    return (
        <>
            <div className="flex flex-col items-center text-primary-blue-900">
                <div className="w-full max-w-7xl p-2">
                    ...
                    <div className="w-full flex justify-center">
                        <Button href={`/learner/courses/${courseId}/preview`} className="mt-16">
                            Return to Preview
                        </Button>
                    </div>
                </div>
